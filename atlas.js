const BUILD = "v3.18";
    ...
    function spriteStyle(id, size) {
      ...
      var sw = (SPRITE_SHEET.cols * SPRITE_SHEET.cell * s);
      var sh = (SPRITE_SHEET.rows * SPRITE_SHEET.cell * s);
      return "--ssw:" + sw + "px;--ssh:" + sh + "px;--sx:-" + (a.x * s) + "px;--sy:-" + (a.y * s) + "px;" +
        "background-image:url(" + SPRITE_SHEET.url + ");background-repeat:no-repeat;" +
        "background-size:" + sw + "px " + sh + "px;background-position:-" + (a.x * s) + "px -" + (a.y * s) + "px;";
    }
  ...
  "2020s": {
    ...
    function renderTimeline(filtered) {
      ...
      placed.forEach(({ mp, lane }) => {
        ...
        node.innerHTML = `
          ...
          <p class="biz-org">NZ Parliament</p>
    ...
    const RELEASE_DATE = "22 Aug 2026";
        const RELEASE_LOG_URLS = [
      "release-log.json",
      "https://raw.githubusercontent.com/FlavourThink/nz-politics/main/release-log.json"
    ];
    function renderReleaseLog() {
      var body = document.getElementById("releaseLogBody");
      if (!body) return;
      if (window.__releaseLogData) {
        paintReleaseLog(window.__releaseLogData);
        return;
      }
      body.innerHTML = "<p class=\"poll-legend\">Loading release log…</p>";
      (function tryUrl(n) {
        if (n >= RELEASE_LOG_URLS.length) {
          body.innerHTML = "<p class=\"poll-legend\">Release log could not be loaded. Check release-log.json next to this page.</p>";
          return;
        }
        fetch(RELEASE_LOG_URLS[n], { cache: "no-store" }).then(function(r) {
          if (!r.ok) throw new Error(r.status);
          return r.json();
        }).then(function(data) {
          window.__releaseLogData = data;
          paintReleaseLog(data);
        }).catch(function() { tryUrl(n + 1); });
      })(0);
    }
    function paintReleaseLog(data) {
      var body = document.getElementById("releaseLogBody");
      if (!body) return;
      ...
      body.innerHTML = list.map(function(entry, i) {
        ...
        return '<div class="rel-entry' + (i === 0 ? " current" : "") + '"><div class="rel-head"><span class="rel-ver">' +
          entry.version + '</span><span class="rel-date">' + (entry.date || "") + "</span></div><ul>" + items + "</ul></div>";
      }).join("") || "<p class=\"poll-legend\">No entries.</p>";
    }
    ...
    function renderFeatureBoard() {
      ...
      function paint(data) {
        ...
        box.innerHTML = rows.length
          ? rows.map(function(f) {
              return '<li><div class="feat-copy"><strong>' + (f.title || "") + "</strong><span>" + (f.detail || f.body || "") +
                (f.status ? " · " + f.status : "") + "</span></div></li>";
            }).join("")
          : '<li class="poll-legend">No requests listed yet.</li>';
      }
      if (window.__featureRequests) { paint(window.__featureRequests); return; }
      box.innerHTML = '<li class="poll-legend">Loading…</li>';
      (function tryUrl(n) {
        if (n >= urls.length) {
          box.innerHTML = '<li class="poll-legend">Could not load feature-requests.json.</li>';
          return;
        }
        fetch(urls[n], { cache: "no-store" }).then(function(r) {
          if (!r.ok) throw new Error(r.status);
          return r.json();
        }).then(function(data) {
          window.__featureRequests = data;
          paint(data);
        }).catch(function() { tryUrl(n + 1); });
      })(0);
    }
    ...
    var closeFeatureTop = document.getElementById("closeFeatureTop");
    if (closeFeatureTop) closeFeatureTop.addEventListener("click", closeFeaturePage);
